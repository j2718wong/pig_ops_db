...
DROP PROCEDURE IF EXISTS pig_dead_type_add $$
CREATE PROCEDURE pig_dead_type_add(
    in_user_id              INT,
...
IF language_id = LANGUAGE_ID_ENGLISH THEN 
    SELECT  id
    INTO    cur_pig_dead_type_id
    FROM    pig_dead_type
    WHERE   account_id          = cur_user_account_id   AND
            UPPER(name)         = UPPER(in_name)
    LIMIT   1;

    IF cur_pig_dead_type_id > 0 THEN 
...
ELSE
    ...
    WHERE   business_object_id = BUSINESS_OBJ_ID_PIG_DEAD_TYPE AND
            ...
            UPPER(translated_text) = UPPER(in_name);
            
    IF cur_translation_id > 0 THEN 
        SET res_num     = RES_NUM_DUPLICATE_ENTRY;
        SET res_code    = "RES_NUM_DUPLICATE_ENTRY";
